...
void SleepBlockStart(const SLEEP_EnergyMode_t Energy_Mode)
{
...
void SleepBlockEnd(const SLEEP_EnergyMode_t Energy_Mode)
{
	CORE_DECLARE_IRQ_STATE;
	CORE_ENTER_CRITICAL();

	if(Energy_Mode == sleepEM1)
		SLEEP_SleepBlockEnd(sleepEM2); 	//Block EM2 and higher
	else
		SLEEP_SleepBlockEnd(sleepEM3);	//Block EM3 and higher
