...
/*	  delayApproxOneSecond();
	  ...
	  /*
	  if(sleep_mode_blocked == sleepEM3)
	  {*/
		  /* Force to EM3 */
		/*  EMU_EnterEM3(true);
	  }*/

	  /*if((sleep_mode_blocked == sleepEM1) || (sleep_mode_blocked == sleepEM2))
	  {*/
		  /* Sleep in EM1 and EM2 */
	/*	  SLEEP_Sleep();
	  }*/

	  SLEEP_Sleep();

	  if(Interrupt_Event_Bitmask == 0x1)
	  {
		  temperature = ReadTemperature();
		  LOG_INFO("Temperature is %.2f C\n", temperature);
		  Interrupt_Event_Bitmask &= ~(0x01);
	  }
