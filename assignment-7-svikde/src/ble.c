...
void GeckoUpdate(struct gecko_cmd_packet* evt)
{
	...
	/* Service */
	HealthTermometerUUID.data[0] = 0x09;
	HealthTermometerUUID.data[1] = 0x18;
	HealthTermometerUUID.len = 2;

	/* Characteristic */
	TemperatureMeasurementUUID.data[0] = 0x1C;
	TemperatureMeasurementUUID.data[1] = 0x2A;
	TemperatureMeasurementUUID.len = 2;
	...
	switch(BGLIB_MSG_ID(evt->header))
	{
		...
		case gecko_evt_le_connection_opened_id:
			...
			BTSTACK_CHECK_RESPONSE(gecko_cmd_le_connection_set_timing_parameters(con_handle,60,60,3,600,0,0));
